...
const ShayriCard = ({ shayri, poetImage, isFavoritePage }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3 }}
      className="glass rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 border border-slate-100 flex flex-col justify-between h-full bg-white/70"
    >
      <div className="space-y-3">
          ...
          <div className="flex flex-col items-end space-y-2">
            {isFavoritePage ? (
               <button 
                ...
                className="bg-red-50 text-red-500 border border-red-100 hover:bg-red-100 px-3 py-1 rounded-lg text-xs font-bold transition-all shadow-sm"
               >
                 ❌ Remove
               </button>
            ) : (
            ...
            <div className="flex bg-slate-50 rounded-md p-1 border border-slate-100 scale-90 origin-right">
              {['roman', 'hindi', 'urdu'].map((l) => (
                <button
                  key={l}
                  onClick={() => setLang(l)}
                  className={`px-1.5 py-0.5 text-[9px] rounded transition-all capitalize font-bold ${
        ...
        <div className="relative min-h-[70px] flex items-center justify-center text-center px-1">
          <AnimatePresence mode="wait">
            <motion.p
              ...
              exit={{ opacity: 0, y: -5 }}
              className={`text-base font-bold tracking-tight leading-relaxed text-slate-800 ${lang === 'urdu' ? 'font-serif' : ''} text-center w-full`}
      ...
      <div className="flex items-center justify-between pt-3 mt-3 border-t border-slate-100/50">
        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{shayri.poet}</span>
