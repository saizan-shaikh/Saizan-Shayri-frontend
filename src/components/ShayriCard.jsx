...
import { Heart, Copy, Download, Share2, Globe, Edit, Trash2 } from 'lucide-react';
    ...
    >
      <div className="space-y-3 md:space-y-4 flex-grow flex flex-col">
        <div className="flex justify-between items-start flex-shrink-0">
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-white shadow-md flex-shrink-0 bg-slate-100 relative">
            <img 
              src={displayImage} 
              alt={shayri.poet} 
            ...
            />
            {user?.role === 'admin' && (
              <div className="absolute inset-0 bg-blue-600/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
          ...
          <div className="flex flex-col items-end space-y-2">
            <div className="flex items-center space-x-2 text-[10px] font-bold text-slate-400">
               <span className="flex items-center"><Globe className="w-3 h-3 mr-1" /> {shayri.views || 0}</span>
               <span className="flex items-center"><Heart className={`w-3 h-3 mr-1 ${shayri.likes > 0 ? 'text-red-500 fill-current' : ''}`} /> {shayri.likes || 0}</span>
            </div>
            {isFavoritePage ? (
               <button 
               ...
               <div className="flex items-center space-x-2">
                 {user?.role === 'admin' && (
                   <div className="flex space-x-1">
                     <button
                      onClick={() => navigate(`/admin/edit-shayri/${shayri._id}`)}
                      className="p-1.5 rounded-lg bg-blue-50 text-blue-600 border border-blue-100 hover:bg-blue-100 transition-colors shadow-sm"
                      title="Edit Shayri"
                     >
                     ...
                     <button
                      ...
                      className="p-1.5 rounded-lg bg-red-50 text-red-500 border border-red-100 hover:bg-red-100 transition-colors shadow-sm"
                      title="Delete Shayri"
                     >
